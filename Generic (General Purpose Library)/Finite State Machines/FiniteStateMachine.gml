...
// Default wildcard transition name
#macro WILDCARD_TRANSITION_NAME_FINITE_STATE_MACHINE_GENERIC \
"*"
...
/// ----------------------------------------------------------------------------
/// @parameter {string} initial_state_name
/// <parameter_description>
///
/// @parameter {boolean} [execute_initial_enter_event]
/// true
///
/// @parameter {type} [event_execution_context]
/// [(Id.Instance|Struct)=other]
///
/// ----------------------------------------------------------------------------
...
function FiniteStateMachine(arguments) constructor {
	...
	/// ----------------------------------------------------------------------------
	/// @description
	/// This function is used to check if the finite state machine has the
	/// specified state.
	/// ----------------------------------------------------------------------------
	...
	/// ----------------------------------------------------------------------------
	/// @function get_state_runtime_frames(parameters)
	/// ----------------------------------------------------------------------------
	/// @description
	...
	/// @return {number}
	/// ----------------------------------------------------------------------------
	get_state_runtime_frames = function(parameters = {}) {
		_us = arguments[$ ""] = true
	...
	/// ----------------------------------------------------------------------------
	/// @function set_state_runtime(parameters)
	/// ----------------------------------------------------------------------------
	/// @description
	/// <function_description>
	/// ----------------------------------------------------------------------------
	...
	/// @return {struct.FiniteStateMachine} self
	/// ----------------------------------------------------------------------------
	set_state_runtime = function(parameters = {}) {
		_time = arguments[$ ""]
		_us = arguments[$ ""] = true
		if (!is_real(_time)) {
	...
	private.history_capacity = max(0, FINITE_STATE_MACHINE_DEFAULT_HISTORY_CAPACITY)
	/// ----------------------------------------------------------------------------
	/// @type {boolean}
	...
	private.fit_history_contents = function() {
		array_resize(private.history, max(2, min(private.history_capacity, array_length(private.history))))
		return self
	}
	...
	/// ----------------------------------------------------------------------------
	/// @return {boolean}
	/// Whether state history is enabled.
	/// ----------------------------------------------------------------------------
	...
	get_history = function() {
		return variable_clone(private.history)
	}
	...
	set_history = function(parameters = {}) {
		...
		if (is_numeric(_history_capacity)) {
			private.history_capacity = max(0, _history_capacity)
			private.fit_history_contents()
		}
